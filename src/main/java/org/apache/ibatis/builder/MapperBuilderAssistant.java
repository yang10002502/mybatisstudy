...
public class MapperBuilderAssistant extends BaseBuilder {
  ...
  public MappedStatement addMappedStatement(
    ...
    MappedStatement.Builder statementBuilder = new MappedStatement.Builder(configuration, id, sqlSource, sqlCommandType)
      ...
      .cache(currentCache);
    // 从configuration中获取已经解析好的ParameterMap对象
    ParameterMap statementParameterMap = getStatementParameterMap(parameterMap, parameterType, id);
    if (statementParameterMap != null) {
      // 对statementBuilder进行parameterMap进行赋值
      statementBuilder.parameterMap(statementParameterMap);
    }

    MappedStatement statement = statementBuilder.build();
    // 将增删查改的标签解析成MappedStatement，并添加至configuration中Map对象mappedStatements中，
    // key为statement的ID，value为statement
  ...
  private ParameterMap getStatementParameterMap(String parameterMapName, Class<?> parameterTypeClass, String statementId) {
    // 设置parameterMapName为mapper接口全路径和parameterMapName的拼接
    parameterMapName = applyCurrentNamespace(parameterMapName, true);
    ParameterMap parameterMap = null;
    if (parameterMapName != null) {
      try {
        // 从configuration中获取parameterMap对象
